...
class RFIDCommandSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        RFIDAuthorizationQueue.objects.create(
            ...
            command_string=formatted_string.rstrip(),  # remove \r\n when saving to DB
            is_sent=False
        )
        # ➕ Save main command
        ...
        # 🔁 Auto-add CheckStatus command for Authorize or Unauthorize
        if action in ["Authorize", "Unauthorize"]:
            check_code = map_action_to_code("CheckStatus")
            rfid_str = ",".join(rfid_list)
            check_body = f",{device_id},{check_code},{rfid_str}"
            length = len(check_body)
            cumulative = f"$$:{length}{check_body}"
            checksum = calculate_checksum(cumulative)
            check_command_string = f"{cumulative}{checksum}\r\n"

            # Save CheckStatus to queue and env (without \r\n in DB)
            RFIDAuthorizationQueue.objects.create(
                device_id=device_id,
                vehicle_id=vehicle_id,
                command_string=check_command_string.rstrip(),
                is_sent=False
            )
            add_command_to_env(check_command_string, device_id, vehicle_id)
        ...
        return RFIDCommandLog.objects.create(
            ...
            rfid_list=rfid_list,
            action=action,
            command_code=self.cleaned_data["command_code"],
            formatted_string=formatted_string  
        )
